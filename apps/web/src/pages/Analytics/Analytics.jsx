...
const Analytics = () => {
    ...
    const StatCard = ({ icon: Icon, label, value, color, borderColor }) => (
        <div className={`bg-[#1e293b] p-5 rounded-xl border ${borderColor}`}>
            ...
            <p className={`text-3xl font-bold ${color === 'text-slate-400' ? 'text-text-primary' : color}`}>{value}</p>
            ...
            <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-gradient-to-br from-emerald-600 to-teal-600 p-8 rounded-2xl shadow-lg shadow-emerald-500/20">
                        <DollarSign className="text-text-primary/80 mb-4" size={40} />
                        <p className="text-text-primary/80 text-sm mb-2">Total Bounty Pool</p>
                        <p className="text-5xl font-bold text-text-primary">{stats.totalBounty}</p>
                        <p className="text-text-primary/80 mt-1">ETH</p>
                    </div>

                    <div className="bg-gradient-to-br from-indigo-600 to-purple-600 p-8 rounded-2xl shadow-lg shadow-indigo-500/20">
                        <BarChart3 className="text-text-primary/80 mb-4" size={40} />
                        <p className="text-text-primary/80 text-sm mb-2">Average Bounty</p>
                        <p className="text-5xl font-bold text-text-primary">{stats.averageBounty}</p>
                        <p className="text-text-primary/80 mt-1">ETH per job</p>
                    </div>
                </div>
                            ...
                            stats.recentActivity.map((issue) => (
                                <div key={issue._id} className="flex items-center gap-4 p-4 bg-[#0f172a] rounded-xl border border-[#334155] hover:border-[#475569] transition-colors">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-semibold text-text-primary truncate">{issue.title}</p>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${issue.status === 'OPEN' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
